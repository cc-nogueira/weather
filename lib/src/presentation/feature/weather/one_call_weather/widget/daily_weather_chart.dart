...
class DailyWeatherChart extends ChartWidget with ColorRangeMixin, TemperatureMixin, WeatherMixin {
  const DailyWeatherChart({super.key, required this.weather, super.margin, super.padding}) : super(height: chartHeight);
  ...
  Widget? _chartTitle(BuildContext context) {
    ...
    return Text(translations.daily_chart_title, style: titleStyle(context), textScaler: TextScaler.linear(1.2));
  }
    ...
    return [
      ...
      ScatterSeries<DailyWeather, DateTime>(
        ...
        dataLabelSettings: DataLabelSettings(
          ...
          builder:
              (dynamic item, dynamic point, dynamic series, int pointIndex, int seriesIndex) => _dataLabel(item, unit),
        ),
      ),
    ];
  }
  ...
  ChartAxis _primaryYAxis(BuildContext context, List<DailyWeather> data) =>
      NumericAxis(isVisible: false, minimum: 0.0, maximum: 10.0, anchorRangeToVisiblePoints: false);
