          ...
          child: Center(
            child: Container(
              decoration: BoxDecoration(
                  color: theme.scaffoldBackgroundColor, border: Border.all(color: theme.colorScheme.onSurface)),
              ...
              child: Column(
                ...
                children: [
                  Padding(
                    ...
                    child: Text(Translations.of(context)!.scale_chart_header_sample_message, style: txtSt),
                  ),
...
class _TemperatureScaleChart extends ScaleChart<Temperature> with TemperatureMixin {
  ...
  double _temperatureInChartUnit(double value) => Temperature.degreesCelcius(value).convertTo(unit).amount;
  ...
  ChartLabelFormatterCallback? get primaryYAxisLabelFormatter => (AxisLabelRenderDetails details) {
        ...
        if (unit == celciusUnit) {
          ...
          amountInCelcius = Quantity(amount: details.value.toDouble(), unit: unit).convertTo(celciusUnit).amount;
        }
